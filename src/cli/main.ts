...
import { TextbeltFreeProvider } from "../notifications/providers/textbelt.js";
...
import type { MonitorConfig, SchedulerConfig, UsageSnapshot } from "../types.js";
import { Logger } from "../utils/logger.js";
import { findExecutable, runCommand } from "../utils/process.js";
import { describeWindow, localTime, maskPhone } from "./format.js";
...
Commands:
  ...
  test-sms [--force]   Send an explicit Textbelt test SMS
  test-sms --validate  Validate the provider request without consuming SMS quota
  start | stop | restart
...
async function printSuccess(config: MonitorConfig, snapshot: UsageSnapshot, dryRun: boolean): Promise<void> {
  ...
  console.log(`Scheduled reset SMS: ${config.notifyScheduled ? "on" : "off"}`);
  console.log(`Unexpected reset SMS: ${config.notifyUnexpected ? "on" : "off"}`);
  console.log(`Next advertised weekly reset: ${localTime(weekly.sort((a, b) => a.resetsAt - b.resetsAt)[0]?.resetsAt)}`);
  console.log("Useful commands: codex-reset-watch status | configure | check | test-sms");
  console.log("No test SMS was sent.");
}
...
async function installCommand(args: string[]): Promise<void> {
  ...
  try {
    ...
    const initial = await installChoices(prompt);
    const customize = initial.customize;
    console.log("\nVerifying Codex CLI and ChatGPT authentication...");
    const codex = await codexSource(prompt);
    const snapshot = await codex.source.read();
    const settings = await settingsFromSnapshot(prompt, snapshot, initial, customize);
    ...
    await printSuccess(config, snapshot, dryRun);
  } finally {
    prompt.close();
  }
}
...
async function statusCommand(): Promise<void> {
  ...
  if (config.provider === "web-push") {
    console.log(`Paired iPhone: ${config.webPush ? "yes" : "no"}`);
    if (config.webPush) console.log(`Setup app: ${config.webPush.setupUrl}`);
  } else {
...
async function testSmsCommand(args: string[]): Promise<void> {
  const config = await requireConfig();
  const provider = new TextbeltFreeProvider();
  if (args.includes("--validate")) {
    const result = await provider.validateWithoutSending(config.phone);
    console.log(result.status === "sent" ? "Textbelt accepted the validation request; no SMS quota was consumed." : `Validation failed: ${result.error}`);
    if (result.status !== "sent") process.exitCode = 1;
    return;
  }
  console.log("This will consume today's free Textbelt message.");
  if (!args.includes("--force")) {
    const prompt = new Prompt();
    try { if (!await prompt.confirm("Send the test SMS", false)) { console.log("Cancelled."); return; } }
    finally { prompt.close(); }
  }
  console.log(`Sending a test SMS to ${maskPhone(config.phone)} through Textbelt Free.`);
  console.log("Keep this command open until a final result appears. Retries run here and are not queued in a separate background process.");
  const result = await sendWithRetry(provider, {
    id: `test-${Date.now()}`,
    message: "Codex Reset Watch: setup test. Reply STOP to unsubscribe."
  }, [{ phone: config.phone }], {
    onAttempt(attempt, totalAttempts) {
      console.log(`Textbelt attempt ${attempt} of ${totalAttempts}...`);
    },
    onRetry(details) {
      console.log(`Attempt ${details.attempt} failed: ${details.error}`);
      console.log(`Retrying in ${Math.ceil(details.delayMs / 1000)} seconds. Closing now will cancel the remaining retries.`);
    }
  });
  if (result.status === "sent") {
    console.log(`Textbelt accepted the test SMS${result.providerMessageId ? ` (message ID ${result.providerMessageId})` : ""}.`);
    console.log("The request succeeded and you may close this command. Carrier delivery remains best effort.");
  } else {
    console.log(`Final result: test SMS ${result.status}: ${result.error ?? "unknown error"}`);
...
async function setupWebPushCommand(args: string[]): Promise<void> {
  ...
  const keys = config.webPush
    ? { publicKey: config.webPush.vapidPublicKey, privateKey: config.webPush.vapidPrivateKey }
    : generateVapidKeys();
  const pairingUrl = new URL(setupUrl);
  pairingUrl.hash = `vapid=${encodeURIComponent(keys.publicKey)}`;

  console.log("\niPhone Web Push setup");
  console.log("  Web Push has no per-message fee and does not use Textbelt.");
  ...
  try {
    ...
    const subscription = decodeSubscriptionCode(code);
    ...
    await ensurePrivateDirectory(paths.logDir);
    await scheduler.install(schedulerConfig(updated));
    console.log("iOS Web Push is paired and the scheduled monitor now uses it.");
...
async function sendTestPush(config: MonitorConfig): Promise<void> {
  if (config.provider !== "web-push" || !config.webPush) {
    throw new Error("iOS Web Push is not configured. Run `codex-reset-watch setup-web-push` first.");
  }
...
async function main(): Promise<void> {
  ...
  switch (command) {
    ...
    case "test-sms": await testSmsCommand(args); break;
